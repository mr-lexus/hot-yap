...
Этот документ описывает подготовленную схему публикации HotYap / «Горячего Лая»: где находится лендинг, как он собирается и индексируется, как GitHub Actions создаёт установщики, как выпустить alpha-релиз и какие ограничения пока остаются.
